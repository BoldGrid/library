...
class Test_BoldGrid_Library_Library_Plugin_Plugins extends WP_UnitTestCase {
	...
	public function test_getActivePluginBySlug() {
		$all_plugins = $this->plugins->getAllPlugins();
		$x           = 0;

		foreach( $this->expected_plugin_slugs as $expected_plugin_slug ) {
			$plugin = $this->plugins->getActivePluginBySlug( $all_plugins, $expected_plugin_slug );
		...
		$this->assertTrue( is_wp_error( $this->plugins->getActivePluginBySlug( $all_plugins, 'FakeFake' ) ) );
	}
